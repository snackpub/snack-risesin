...
import com.risesin.core.log.dao.SysLogUsualDao;
 ...
 */
@Service
public class LogUsualServiceImpl extends RisesinBaseServiceImpl<SysLogUsualDao, LogUsual, Integer> implements ILogUsualService {

    @Override
    public Page<LogUsual> findSearch(Map whereMap, Query query) {
        Specification<LogUsual> specification = createSpecification(whereMap);
        PageRequest pageRequest = QueryUtil.getPage(query);
        return baseDao.findAll(specification, pageRequest);
    }

    public List<LogUsual> findSearch(Map whereMap) {
        Specification<LogUsual> specification = createSpecification(whereMap);
     ...
     * @return
     */
    private Specification<LogUsual> createSpecification(Map searchMap) {

        return new Specification<LogUsual>() {

            @Override
            public Predicate toPredicate(Root<LogUsual> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
